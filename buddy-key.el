;;; buddy-key.el --- a coy of luna-key(General.el for Lunarymacs)      -*- lexical-binding: t; -*-

;; Copy and Modify from https://github.com/casouri/lunarymacs/edit/master/site-lisp/luna-key.el
;; Origin Author: Yuan Fu <casouri@gmail.com>
;; All rights belong to Origin Author

;;; This file is NOT part of GNU Emacs

;;; Commentary:
;;
;; Home-brewed general.el. Though `buddy-def-key' has some differences
...
(defvar buddy-key-preset-alist nil
  "Stores defined presets.")

(defvar buddy-key-override-mode-map (make-sparse-keymap)
  "One map to rule them all.")

(defvar buddy-key-override-map-alist
  `(buddy-key-override-mode . ,buddy-key-override-mode-map)
  "Alist put into `emulation-mode-map-alists' to override all other maps.")

(defvar buddy-key-postponed-alist nil
...
(defun buddy-key-define-postponed-binding (_)
  "Define postponed bindings in ‘buddy-key-postponed-alist’."
  (dolist (map (mapcar #'car buddy-key-postponed-alist))
    (when (boundp map)
      (pcase-dolist (`(,key ,def)
                     (alist-get map buddy-key-postponed-alist))
        (define-key (symbol-value map) key def))
      (setf (alist-get map buddy-key-postponed-alist) nil))))

(define-minor-mode buddy-key-override-mode
  "Minor mode used to activate our override keymap."
  :global t
  :lighter ""
  :keymap 'buddy-key-override-mode-map
  (if buddy-key-override-mode
      (unless (memq 'buddy-key-override-map-alist
                    emulation-mode-map-alists)
        (push 'buddy-key-override-map-alist emulation-mode-map-alists))
    (setq emulation-mode-map-alists
          (remove 'buddy-key-override-map-alist
                  emulation-mode-map-alists))))

(defun buddy-key-def-preset (preset &rest args)
  "Define PRESET as ARGS.

ARGS can be anything valid in `buddy-def-key'.
...
is equivalent to
    ...
    (buddy-def-key
     ...
     KEY DEF)"
  (declare (indent 1))
  (setf (alist-get preset buddy-key-preset-alist) args))

(defun buddy-key-normalize (prefix key)
...
(defun buddy-key-define (key def &optional map-list prefix condition)
  "Define KEY to DEF.
...
to non-nil."
  (let ((map-list (or map-list (list 'global-map)))
        (key (buddy-key-normalize prefix key))
            ...
            (if desc
              ...
              `(menu-item "" ,def
                          :filter ,(lambda (cmd)
                                     (if (funcall condition) cmd))))))
    (dolist (map map-list)
      (let ((map (if (eq map 'override) 'buddy-key-override-mode-map map)))
        ...
        (if (boundp map)
          ...
          (add-hook 'after-load-functions
                    #'buddy-key-define-postponed-binding))))))

(defun buddy-def-key (&rest args)
  "Define key.
...
keymap defined by buddy-key, or a list of these three forms. KEY
and DEF can be anything that `define-key' accepts. `kbd' is
automatically added to KEY but not DEF.

You can also use preset modifiers defined by `buddy-key-def-preset'.
      ...
      (let (arg map-list prefix condition)
        (while args
          ...
          (pcase arg
            ...
            ;; Preset modifiers.
            ((pred keywordp)
             (when-let ((preset (alist-get arg buddy-key-preset-alist)))
               (setq args (append preset args))))
            ;; Next two arguments are key and value.
            (_ (let ((key arg)
                     (def (pop args)))
                 (buddy-key-define key def map-list prefix condition))))))
    (setting-constant (error "Not enough arguments"))))

(provide 'buddy-key)

;;; buddy-key.el ends here
